...
<header class="bg-trueGray-700 sticky z-50 top-0" x-data="dropdown()">
    <div class="container-menu flex items-center h-16 justify-between md:justify-start">
        <a dusk="categorias" :class="{'bg-opacity-100 text-orange-500': open}" x-on:click="show()" class="flex flex-col items-center justify-center order-last md:order-first px-6 sm:px-4 bg-white bg-opacity-25 text-white cursor-pointer font-semibold h-full">
                    ...
                    @else
                        <x-jet-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                ...
                                <x-jet-dropdown-link href="{{ route('register') }}">{{ __('Registro') }}</x-jet-dropdown-link>
                            </x-slot>
                        </x-jet-dropdown>
                    @endauth
                </div>
